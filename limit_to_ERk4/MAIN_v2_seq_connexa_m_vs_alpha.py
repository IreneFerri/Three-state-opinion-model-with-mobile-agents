...
nsteps = N0*50  # Maximum number of steps even if the stady state has not been reached
...
alpha_min = 0.00 #RANG ALPHA INTERESSANT: 0-1
alpha_max = 2.00
siz_nod = 40 #for some plots
L = 1 #table size (unchangeble btm)
stable_condition = int(50*np.log(N))  # To Consider reached steady state
#stable_condition = 30
...
res1_filename = 'num1_T'+str(temp)+'_N'+str(N)+'T'+str(temp)+'.csv'
res0_filename = 'num1_T'+str(temp)+'_N'+str(N)+'T'+str(temp)+'.csv'
res_1_filename = 'num1_T'+str(temp)+'_N'+str(N)+'T'+str(temp)+'.csv'
...
if (alpha <=1):  # Ground state polarized or the tree of them
  spins=np.ones(N)
else:   # Ground state neutral
  spins=np.zeros(N)
...
#  G_main = energy_function(G_main,vectors,R,N)[1]
...
print('GROUND STATE', energy, num1, num0, num_1)
...
# ********************************************************************************************************
# **************************************************************************************************
num1_array = np.zeros((N_of_repetitions, N_of_trials))
num0_array = np.zeros((N_of_repetitions, N_of_trials))
num_1_array = np.zeros((N_of_repetitions, N_of_trials))
...
for rep in range(N_of_repetitions):
    ...
    N = N0 #number of agents  at the beggining
    G_main = nx.Graph()
#    G_main = nx.create_empty_copy(G_main)
#    np.random.seed(seed=4)
    orient = []
    for a in range(N): #creates the nodes and a dictionary that includes the position of every node
        G_main.add_node(a)
...
#
#    Check stability i.e. if energy remains stable
          if (stability < stable_condition):
            energy_prev = energy                
#
            G = nx.create_empty_copy(G)
            positions = one_random_walk_step(v,positions,N)
            nx.set_node_attributes(G,positions,'posis') #now every node has an associated position ;   G_main will save the initial configuration so it can be used for all the velocities
#   Reshuffle nodelist, so at every MC step we a certain we take EVERY node, but in a different order
...
# CHECH IF NECESSARY :
#            print('step = ', step, energy)  ---> It seems it's not at v = 0, but we keep it for finite velocities
...
######################################################
######################################################
# Store magnet and num neutrals
        num1_array[rep, trial] = num1
        num0_array[rep, trial] = num0
        num_1_array[rep, trial] = num_1
####################################################################################################################
        alpha = alpha + alpha_step


####################################################################################################################
## Plot the graph  (Last configuration) 
#    imagename = 'RW_video_N'+str(N).zfill(4)+'_T'+str(temp)+'_a'+str(alpha)+'_k'+str(ave_k)+'_v'+str(v)+'_t'+str(step)+'_R'+str(rep)+'.png'
#    fig, ax = plt.subplots(1)
#    fig.set_size_inches(9, 9)
#    ax.set_aspect('equal', adjustable='box')
#    ax.set_title('N = '+str(N)+' - k = '+str(ave_k)+r' - $\alpha = $'+str(alpha) + ' - T = '+str(temp), fontsize = 10)
#    ax.text(0.97, 0.97, 't = '+str(step))
##
#    for i in range (N):
#      if G.nodes[i]['orientation'] == -1:   # RED
...
#    plt.close()
#########################################################################################################################
# Save num agents in each state into an external file
num1_array.tofile(res1_filename, sep='', format='%s')#
num0_array.tofile(res0_filename, sep='', format='%s')#
num_1_array.tofile(res_1_filename, sep='', format='%s')#
#    num_links = G.number_of_edges()
