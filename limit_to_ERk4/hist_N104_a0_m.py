...
print(N, L, ave_k)
###############################################################
n1 = pd.read_csv(filename, delim_whitespace = True,header=0,   usecols = [4]).values.T.reshape(length)
n_1 = pd.read_csv(filename, delim_whitespace = True,header=0,   usecols = [6]).values.T.reshape(length)
magnet = abs(n1 - n_1)
print(magnet)
