...
Nb = 100  # number of bins for the histogram
###############################################################
# READ THE DATA
###############################################################
###############################################################
...
print(filename)
###############################################################
number = pd.read_csv(filename, delim_whitespace = True,header=0,   usecols = [0]).values.T.reshape(length)
N = int(number[0])
links = pd.read_csv(filename, delim_whitespace = True,header=0,   usecols = [1]).values.T.reshape(length)
L = int(links[0])
ave_k = 2*L/N
print(N, L, ave_k)
###############################################################
Pn0 = pd.read_csv(filename, delim_whitespace = True,header=0,   usecols = [5]).values.T.reshape(length)
print(Pn0)
n0_max = np.amax(Pn0)
#
#################################
...
#
fig, ax = plt.subplots()
fig_name = '../../../Documents/Doc/Recerca/Three-state-mobility/images/RW_n0_N'+str(N)+'_a'+str(alpha)+'_glauber.pdf'
plt.title(r'$\alpha = $'+str(alpha), fontsize=15)
plt.xlabel(r'$n_0$', fontsize=15)
plt.ylabel(r'P($n_0$)', fontsize=15)
...
plt.plot(n0, hist,  label=r'$\langle k \rangle$ = 2')
#
