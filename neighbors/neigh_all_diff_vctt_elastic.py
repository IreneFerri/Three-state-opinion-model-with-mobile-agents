...
nsteps = N
vmin = 0.09 #initial velocity module
vmax = 0.10
siz_nod = 40
...
#
prob_file = open('diff_neigh_RW'+'_N'+str(N)+'_v'+"%5.3f"%vmin+'.csv','w')
#
...
#           
            previous_neighbors_list= []
            for a in range (N): 
              previous_neighbors_list.append(neighbors(a, R, G))  # previous First Neighbors (list of N lists)
            G.remove_edges_from(G.edges())  # Remove previous edges
# ************************************************************************
# ************************ MOVE NOVES ************************************************ 
            G = one_random_walk_step(G)     # Move nodes *******************************
            ...
            for a in range (N):           # Every MC step has N flip attempts
              ...
              counter_tot = len(neigh_list)  # counter for the number of nodes which were sencond neighs of "a" in the previous step
              counter_P = 0   # Counter for the number of nodes which are first neighs of "a" in this step AND NOT first in the previous one
              for my_neigh in neigh_list:
                if my_neigh not in previous_neighbors_list[a]: # If the neigh was not neigh in the previous step (new neigh)
