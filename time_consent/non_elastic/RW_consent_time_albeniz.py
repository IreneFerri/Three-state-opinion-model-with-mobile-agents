...
nsteps = 50*N
vmin = 0.35 #initial velocity module
vmax = 0.36
temp=0.05 #TEMPERATURA NO TOQUIS
alpha = 0.00 #PROVAR AMB ALPHA 0 I ALPHA 1
siz_nod = 40
...
fails = open('fails_RW'+'_N'+str(N)+'_a'+str(alpha)+'_T'+str(temp)+'_v'+str(vmin)+'_albeniz.csv','w')
consent_file=open('RW'+'_N'+str(N)+'_a'+str(alpha)+'_T'+str(temp)+'_v'+str(vmin)+'_albeniz.csv','w')
...
#            G_aux = nx.Graph.copy(G) #auxiliar graph. it will contain the NEW states
            G_aux = nx.create_empty_copy(G)
            positions = one_random_walk_step(v,positions,N)
            nx.set_node_attributes(G_aux,positions,'posis') #now every node has an associated position ;   G_main will save the initial configuration so it can be used for all the velocities
