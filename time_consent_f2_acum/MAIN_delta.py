...
nsteps = 5000
vmin = 0.25 #initial velocity module
vmax = 0.3
...
time_data_partial_polar = open('time_partial_polar_v'+str(vmin)+'_a'+str(alpha)+'.csv', 'w')
time_data_partial_neutral = open('time_partial_neutral_v'+str(vmin)+'_a'+str(alpha)+'.csv', 'w')
...
def one_random_walk_step(v,positions,N,G,consent): #now v is a N-vector
    dict = {}
    for node in range(N):
        ...
        if majoria == G.nodes[node]['orientation']: #la majoria son com ell
            ...
            v[node] = v[node]
#AINA
#    consent = all(i <= 5*10**(-3) for i in v)
# Irene 1
#    consent = True
#    for i  in range(N):
#      if (v[i] > 10**(-3)):
...
#
    for a in range(N):
        ...
        deltax = v[a]*np.cos(alpha)
        deltay = v[a]*np.sin(alpha)
        new_x = x + deltax
        new_y = y + deltay
...
def visualitzacio(G,step,N,vel,alpha,filenames):
    ...
    filename = 'N' + str(N) + 'rep' + str(rep) + '_v' + str("{:.2f}".format(vel)) + '_a' + str(alpha) + 'step_' + str(step) + '.png'
...
def crea_subgraph(G):
    ...
    for node in G:
        if G.nodes[node]['orientation'] == 1:
            G_negatius.remove_node(node)
            G_neutres.remove_node(node)
        elif G.nodes[node]['orientation'] == -1:
            G_positius.remove_node(node)
            G_neutres.remove_node(node)
        else:
            G_negatius.remove_node(node)
            G_positius.remove_node(node)
    return G_positius, G_negatius, G_neutres
...
steps_array_neutral = []
steps_array_partial_polar = []
steps_array_partial_neutral = []
steps_array_polar = []
...
#
            ...
            if step == nsteps-1:
                ...
                print('num0 = ', num0, 'magnet = ', abs(num1 - num_1))
                print(sum(v)/N)
                for i in range(N):
                  print(G.nodes[i]['orientation'], v[i])
                break
            ...
            if consent == True: #velocitat nula
                ...
                magne.append(global_magnetization)
                if (num0 == 0):
                  steps_array_partial_polar.append(step)
                else:
                  steps_array_partial_neutral.append(step)
    ...
    # for a in magne:
    #     file.write(str(a) + '\t')
    # for a in largest_list_polar:
    #     if a != 0:
    #         size_polar.write(str(a) + '\t')
    #     else:
    #         size_polar.write('NaN' + '\t')
    # for a in largest_list_neutral:
    #     if a != 0:
    #         size_neutral.write(str(a) + '\t')
    #     else:
    #         size_neutral.write('NaN' + '\t')
    #
    # results.write('\n')
    # file.write('\n')
    # size_polar.write('\n')
...
steps_array_partial_polar = np.array(steps_array_partial_polar)
steps_array_partial_neutral = np.array(steps_array_partial_neutral)
...
for i in range(steps_array_partial_polar.size):
  time_data_partial_polar.write(str(steps_array_partial_polar[i]) + '\n')

for i in range(steps_array_partial_neutral.size):
  time_data_partial_neutral.write(str(steps_array_partial_neutral[i]) + '\n')
...
time_data_partial_polar.close()
time_data_partial_neutral.close()
size_polar.close()
size_neutral.close()
