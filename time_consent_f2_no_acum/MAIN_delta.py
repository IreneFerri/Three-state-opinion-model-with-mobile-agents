...
file=open('magne'+'_N'+str(N)+'_f'+str("{:.2f}".format(factor_frenada))+'_a'+str("{:.2f}".format(alpha))+'_T'+str(temp)+'_vmin'+str(vmin)+'_vmax'+str(vmax)+'.csv','w')
...
for rep in range(N_of_repetitions):
    ...
    for trial in range(N_of_trials): #this loops over the velocities. same initial configurations are used for every v (stored in G_main)
        ...
        #
            ...
            if consent == True: #all nodes are steady
                orient.append(str(num0/N)) #percentatge de neutres
                global_magnetization = abs(num1 - num_1)/N
                magne.append(global_magnetization)
                steps_array_partial.append(steps_array_partial)
                break
...
print(len(steps_array_polar), len(steps_array_neutral), len(steps_array_partial))
